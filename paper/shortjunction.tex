\documentclass[twocolumn, notitlepage, 10pt, aps, floatfix, showpacs, prb, citeautoscript]{revtex4-1}
\usepackage{graphicx}
...
In the short junction limit Andreev bound states have most weight in the superconductor and therefore the conventional approach of integrating out the superconductor and obtaining an effective Hamiltonian of the normal system becomes inefficient due to the strong energy dependence of the effective Hamiltonian.

\comment{MBS in short junction limit}
...
These systems were observed to have a well-developed hard induced gap comparable to the gap in the bare Al~\cite{Chang2015}, and subsequently showed zero bias peaks~\footnote{C.~M.~Marcus et al., (unpublished)} and suppressed splitting of low energy states characteristic to MBS~\cite{Albrecht2016}.
The theoretical description of the response of strongly coupled zero-dimensional NS junctions to magnetic field was analyzed in Ref.~\onlinecite{Cole2015}, where the authors report a strong suppression of the effective $g$-factor, potentially leading to the impossibility of inducing a topological phase at magnetic fields below the Clogston limit~\cite{Clogston1962}.

\comment{Use scattering formalism}
Here we extend the analysis of Ref.~\onlinecite{Cole2015} using the scattering formalism that allows us to capture the nonlinear features of the spectrum, and by considering higher-dimensional systems with translational invariance.
The scattering formalism has been routinely applied to short junctions in mesoscopic physics~\cite{[{For review see }] Beenakker2005}.
...
However, in Majorana literature the use of scattering formalism has been limited~\cite{Cheng2012}.
The equivalent of the scattering formalism using the effective Hamiltonian approach amounts to introducing an effective self-energy $\Sigma(E)$ which has a proper dependence on energy $E$~\cite{Liu2012, Stanescu2013, Peng2015}, and then neglecting the energy term in the nonlinear eigenvalue problem $[H-\Sigma(E)]\psi = E\psi$, as done in e.~g.~Ref.~\onlinecite{Poeyhoenen2016}.
...
\item The localization length $\xi_M$ of MBS does not depend on the superconducting gap, and in optimal conditions it is proportional to the spin-orbit length $l_\textrm{SO}$.
...
\end{itemize}
Our analytical calculations fully coincide with the results obtained using numerical scattering approach to short junctions and exact diagonalization of a discretized tight-binding Hamiltonian.
...
Sec.~\ref{sec:formalism} contains a pedagogical review of scattering formalism for the calculation of Andreev spectrum.
The following section~\ref{sec:jjintro} presents scaling arguments supporting our conclusions.
...
Sec.~\ref{sec:topphases} investigates the Majorana phase diagram and the behavior of the MBS decay length.
...
In Sec.~\ref{sec:orb}, we estimate the orbital effect of the magnetic field by computing the Andeev spectrum in a cylindrical geometry in a thin shell limit.
...
Last section~\ref{sec:conc} sums up our conclusions.
...
We use the case $n=1$ in Sec.~\ref{sec:1stmodel},~\ref{sec:topphases},~\ref{sec:3d} and $n=2$ in Sec.~\ref{sec:orb}.

\comment{Andreev bound states condition.}
The levels with $|E|<\Delta$ are Andreev bound states, i.e. coherent superpositions of electron and hole excitations which occur due to Andreev reflections~\cite{Andreev1963} at the interface between the normal region and the superconducting terminals.
The wave function quantization condition on the wave function requires that the total sequence of scattering events results in a phase shift $2\pi n$.
...
The universal structure of the Andreev reflection matrix, however, does not change in presence of spin-orbit coupling in the superconductor.
...
Throughout the article we work in the latter basis, but explain the relation to the more commonly used spin basis for reference at the end of this section.
...
\end{equation}
Using the block-diagonal structure of $S_N$ we conclude that the normal scattering matrix of holes is the conjugate of the scattering matrix for electrons, at opposite energy and momentum~\cite{Beenakker2015}:
...
Following Ref.~\onlinecite{Beenakker1991}, eliminating $\psi$ from Eq.~\eqref{bound}, and using an expression for a block matrix determinant one immediately arrives to a determinantal equation for the bound state energies:
...
\end{equation}
where the energy argument is suppressed, since $S_e$ is always evaluated at $E=0$.
If there is only a single superconducting terminal, the Andreev reflection matrix $r$ reduces to a phase factor, which fully drops out from the Eq.~\eqref{nsspec}, as required by gauge invariance.
...
The symmetry condition~\eqref{phs}, the equation~\eqref{det} for the Andreev spectrum, eigenproblem for the spectrum in short junction approximation~\eqref{nsspec} are identical in both bases upon replacing $S_e$ and $r$ with $\tilde S_e$ and $\tilde r$.
...
For instance, since the Majorana phase transition happens when Eq.~\eqref{nsspec} has a zero-energy solution, it follows that $B_*$ does not depend on $\Delta$.
...
\end{equation}
where $N$ is the number of transverse bands occupied in the semiconductor, and $\delta$ the typical interband spacing.
...
In realistic nanowires $m_n\sim 10^{-2}m_e$, and $W\approx \SI{100}{\nm}$, resulting in $\delta \approx \SI{3}{\meV}$, much larger than the superconducting gap in aluminum $\Delta_\textrm{Al} \approx \SI{0.2}{\mu\eV}$, which justifies the short junction approximation for sufficiently transparent contacts $T\gtrsim 0.1$.
...
\end{equation}
We observe that if transparency is high and the number of modes is low, then the relative strength of the orbital and Zeeman effects of the magnetic field is a material parameter dependent on the $g$-factor and the effective mass.
For realistic materials this factor is $\mathcal{O}(1)$, which is in line with our results in Sec.~\ref{sec:orb} and~\ref{sec:3d}.

\comment{MBS size is given by $l_\textrm{SO}$.}
Turning to the spatial extent of the MBS $\xi_M$, we observe that it must diverge in the topological regime in absence of spin-orbit coupling.
...
Specifically, $H_\textrm{1D}$ undergoes a topological phase transition when $E_Z^2 = \Delta'^2 + \mu^2$, and therefore $B_*$ explicitly depends on $\Delta'$.
...
The nanowire is oriented along $x$ axis, NS interface is at $y=0$, and the outer boundary of the wire is at $y=W$.
The superconductor occupies the half-space $y<0$.
Neglecting the orbital effect of the magnetic field makes the motion in $z$ direction separable and reduces the problem to a purely two-dimensional geometry, shown in the bottom panel of Fig.~\ref{fig:1stmodel}.

\begin{figure}[t]
\includegraphics[width=0.8\columnwidth]{figures/3d.pdf}
\includegraphics[width=0.8\columnwidth]{figures/interface}
\caption{(Color online) Top panel: nanowire with width $W$ oriented along the $x$ direction and coupled to a bulk superconductor.
The magnetic field is parallel to the wire, while the Rashba electric field points along $z$ direction.
Bottom panel: semiclassical bound state trajectory in the two-dimensional nanowire. Electrons (solid blue) and holes (dashed red) specularly reflect at the boundary with vacuum and undergo Andreev reflection at the interface with the superconductor.
...
The effective electric field generating Rashba spin-orbit coupling is $\bm{\mathcal E}_R= 2m_n\alpha /\hbar g\mu_B\hat z$.
...
To make further analytical progress, we neglect the spin-orbit coupling in $y$ direction $\alpha\sigma_x p_y$.
...
We later verify the validity of this approximation later by computing the exact expression for the topological phase boundary and by including the transverse spin-orbit coupling in all the tight-binding simulations.

The wave function $\psi_n(k_x, y)$ in the nanowire which satisfies the boundary condition $\psi_n(k_x, W) = 0$ has the general form
\begin{equation}
...
We use the wave function continuity at $y=0$ as well as the current conservation condition on the wave function derivative normal to the interface, in $y$ direction:
...
The scattering matrix holds generally at energies bellow the superconducting gap $|E/\Delta|<1$.
In the short junction approximation, $S_e$ is evaluated at Fermi energy $E=0$.
The Andreev bound spectrum follows immediately upon solving the eigenvalue problem~\eqref{nsspec}:
...
In the derivation of the Andreev spectrum~\eqref{andreev} we neglected the effect of spin-orbit interactions in $y$ direction since $W \ll l_\mathrm{SO}$.
...
By integrating the Schr\"odinger equation near the interface $y=0$ we obtain:
\begin{equation}
...
Matching the wave functions at the NS interface and solving the scattering problem results in a condition for closing the excitation gap identical to Eq.~\eqref{toptrans}, but with the modified the scattering phases $r_\pm$~\eqref{scattphase}:
...
To consider a specific example of the system parameters we take InSb nanowires~\cite{Mourik2012} with effective mass $m_n=0.015\,m_e$ (here $m_e$ is the free electron mass), and with spin-orbit length $l_\mathrm{SO}=\hbar^2/m_n\alpha\approx \SI{250}{nm}$.
...
The experiments using high $\Delta$ superconductors such as NbTiN are in a long junction regime allowing to estimate $T$ because the induced superconducting gap is $\approx T \delta$.
...
In most calculations we use $m_\parallel = 10\,m_n$, however our conclusions are not sensitive to this choice (see App.~\ref{sec:app} for details).
...
\begin{figure}[t]
\includegraphics[width=\columnwidth]{figures/spec_comp}
\caption{(Color online) Comparison of the Andreev spectra of a transparent NS junction between analytical short junction predictions (SJT), numerical short junction results including the spin-orbit interaction in $y$ direction (SJN), and exact diagonalization of the BdG Hamiltonian~\eqref{sc} (ED).
The system parameters are chosen as in Sec.~\ref{sec:phys_params}; the longitudinal momentum is either $k_x=0$ (red), or finite so the spectrum stays always gapped (blue).
...
To verify the correctness of the spectrum in the short junction limit, Eq.~\eqref{andreev}, we compare the analytical expressions with dispersion relation calculated using a Hamiltonian~\eqref{danduke} discretized on a square lattice with lattice constant $a=\SI{0.5}{nm}$ and simulated using Kwant package\cite{Groth2014}.
We first numerically obtain the scattering matrix $S_e(k_x)$ of the normal region and use it as an input to the Eq.~\eqref{nsspec} to obtain the dispersion relation of the hybrid system.
...
\begin{figure}
\includegraphics[width=\columnwidth]{figures/index}
...
\begin{figure}[tb]
\centering
\includegraphics[width=\linewidth]{figures/dispersion}
...
\begin{figure}
\includegraphics[width=\columnwidth]{figures/gaps}
...
Exact evaluation of the MBS decay length starting from the Eq.~\eqref{andreev} is not possible because the spectrum in the short junction approximation does not correspond to a local Hamiltonian (the same fact manifests in the complex nonlinear dispersion of the Andreev states).
...
Once again, it is instructive to estimate to estimate $\xi$ using scaling arguments in two regimes: near a topological transition and deep in the topological phase.
...
In order for the self-energy to become local in $x$ coordinate we neglect the transverse dispersion in the superconductor and set $m_\parallel = \infty$.
We then integrate out the superconductor and add a self-energy to the semiconductor.
Finally, similar to Ref.~\onlinecite{Nijholt2016} we perform an eigendecomposition of the translation operator in the $x$ direction at zero energy to obtain the evanescent waves $\psi\propto e^{-\kappa x}$, with $\kappa$ the eigenvalue of the translation operator.
...
The divergence in decay lengths seen in Fig.~\ref{fig:decay_diag} correspond to the topological transitions identical to ones found in Fig.~\ref{fig:gaps}.
...
\begin{figure}[t]
\includegraphics[width=\columnwidth]{figures/decay_length_2d}
...
\end{equation}
Taking the ratio~\eqref{eq:v_k_f}, it follows that the MBS decay length does indeed grow linearly with the magnetic field and spin-orbit length deep in the topological phase [see Fig.~\ref{fig:compAB}], in qualitative agreement with the numerical calculation using Eq.~\eqref{ximnum}.

\begin{figure}[t]
\includegraphics[width=\columnwidth]{figures/compAB}
\caption{
(Color online)
...
Our results for the scaling of $\xi_M$ with $B$ and $l_\textrm{SO}$ agree with the predictions of the phenomenological 1D model both near the topological transition or deep in the topological phase (see e.~g.~Ref.~\onlinecite{Klinovaja2012}), but we find no dependence of $\xi_M$ on $\Delta$.
...
The lattice constant in the tight-binding simulation is \SI{10}{nm}.
Finally, the remaining of model parameters are chosen according to Sec.~\ref{sec:phys_params} and Sec.~\ref{sub:interf}.
...
\begin{figure}[t]
\includegraphics[width=\columnwidth]{figures/finite}
...
At a single end of the nanowire, there is only one MBS in the topological phase ($\SI{1}{T}\lesssim B \lesssim \SI{3}{T}$, for semiconductor width $W=\SI{100}{nm}$) and two in the trivial phase at high field, due to the chiral symmetry.}
\label{fig:finite}
\end{figure}
...
Finally after the system undergoes the second gap closing and enters the trivial phase at $B \approx \SI{3}{\tesla}$ additional low energy state appears due to the presence of chiral symmetry of the Hamiltonian~\eqref{danduke}~\cite{Schnyder2009, Ryu2010, Tewari2012}.
...
This effect does not manifest in the model of Sec.~\ref{sec:1stmodel} when magnetic field points in $x$ direction.
To include the orbital effect we use a thin shell approximation, when the electron wave function in the semiconductor is confined to its surface, similar to the system studied in Ref.~\onlinecite{osca_majorana_2014}.
However unlike Ref.~\onlinecite{osca_majorana_2014} we do not assume a constant induced gap and consider instead the nanowire contacted by a bulk superconductor, as shown in Fig.~\ref{fig:orb}.
...
By doing so we neglect the chance for electrons to tunnel through the superconductor to the other side of the uncovered section, which is justified by the density mismatch between the superconductor and the semiconductor.
The thin shell limit is oversimplified and it overestimates the orbital effect of a magentic field, however it provides an upper bound on the impact of the orbital effect and remains analytically tractable.

\begin{figure}[tb]
\centering{\includegraphics[width=\columnwidth]{figures/orb}}
...
The superconductor covers the wire over an angle $2\pi-\theta_0$, while both the wire and the superconductor are translationally invariant in $x$ direction.
In cylindrical coordinates $(x, \theta)$ the electron Hamiltonian the surface of the nanowire is:
\begin{equation}
...
We assume that the magnetic field if fully screened from the superconductor and choose a gauge where the vector potential $A=0$ in the uncovered part of the surface, while the two superconducting leads have a phase difference $\phi = (2 e/\hbar) \pi B R^2$.
Compared to the previous sections where the treatment was more general, we assume from the start the transparent junction limit, when Fermi velocities at $k_x=0$ are identical in the superconductor and the semiconductor and also neglect the spin-orbit coupling in the transverse direction, as appropriate for $l_\textrm{SO} \gg R \theta_0$.
...
\begin{figure}[t]
\includegraphics[width=\columnwidth]{figures/gaps_orb}
\centering{\includegraphics[width=\columnwidth]{figures/Borb}}
...
The system consists of a semiconductor nanowire infinite in $x$ direction and with a square cross-section contacted by a bulk superconductor occupying $y<0$ half-space  [see Fig.~\ref{fig:1stmodel}(top panel)].

Due to the large Fermi surface mismatch between the superconductor and the semiconductor we neglect the electron dispersion in $x$ and $z$ directions in the superconductor.
...
This ensures that the vector potential is constant in $x$ direction and that it vanishes at the interface with the superconductor.
...
These correspond to the appearance of the additional bands with a different value of $k_z$ and a minimum in the interface transparency.

\begin{figure}
\includegraphics[width=\columnwidth]{figures/phase_diagrams_3d}
...
Panels (a), (c) show results for wire section $\SI{100}{nm} \times \SI{100}{nm}$, while (b), (d), for $\SI{120}{nm} \times \SI{120}{nm}$.
...
Similar to our observations from Sec.~\ref{sec:orb}, the orbital effect of the magnetic field has a strong effect on the shape of the topological phase boundaries and reduces both $\Delta_\textrm{spec}$ and $B_*$ similar to the toy model simulations.
Increasing the cross section of the wire (Fig.~\ref{fig:phase_diagrams_3d}(a), (c), against (b), (d)) confirms that in 3D the critical fields preserve the scaling with $B_*\sim 1/W^2$ independent on the presence or absence of orbital effects.
...
\acknowledgements{
The authors thank B.~van Heck, M.~Irfan, M.~P.~Nowak, T.~\"O.~Rosdahl, and M.~Wimmer for many fruitful discussions.
This research was supported by the Foundation for Fundamental Research on Matter (FOM), the Netherlands Organization for Scientific Research (NWO/OCW) as part of the Frontiers of Nanoscience program, and an ERC Starting Grant.}
...
At a given energy $E$ there are generally two modes in the semiconductor and two spin-degenerate modes in the superconductor with momenta in $y$ direction $k_\pm$ and $q$ respectively:
\begin{eqnarray}
...
\begin{figure}[t]
\includegraphics[width=\columnwidth]{figures/transparency}
\caption{(Color online)
Transmission probability of one of the spin polarizations $T_-$ of an infinite NS junction.
(a, c, d) $T_-$ as a function of magnetic field $B$ in units of $\frac{2\delta}{g\mu_B}$, with $\delta$ defined in Eq.~\eqref{eq:e_thouless}, and parallel momentum $k_x$ (in units of $k_F^0=\sqrt{2m_n\mu_n/\hbar^2}$).
...
In contrast, $T_-$ is always well defined at the Fermi surface for $\mu_n>0$.
